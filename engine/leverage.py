...
**표준 재무 항등식이다.** 대차대조표 항등식(자산 = 부채 + 자기자본)에서 바로
유도되고, 어느 자산군에도 그대로 성립한다 — 전세 낀 아파트든 증거금 낀
선물이든 같은 한 줄이다. 새로 설계한 식이 아니라 정의를 정리한 것이다.

이 식의 요점은 하나다 — **대출은 물건의 수익을 키우지 않는다.**
자산수익률과 조달비용의 **차이에 지렛대를 걸 뿐**이고, 차이가 음수가 되면
같은 배수로 반대로 돈다.

설계 원칙(DISCLAIMER §1): **점추정을 내지 않는다.**
...
def roe(roa: float, multiple: float, funding_rate: float) -> float:
    ...
    아래 예제(자산 1억·보증금 1천만·대출 8천만@3.5%)에서
...
def sign_flip_point(roa: float, loan: float = 0.0, free_debt: float = 0.0) -> float:
    ...
    예제(자산 1억·보증금 1천만·대출 8천만·ROA 4%)에서
...
def critical_recovery(multiple: float) -> float:
    ...
    ⚠ 「임계 하락률」(자산가격이 얼마나 내리면 자기자본이 0이 되나)과
      식은 같으나 **재는 대상이 다르다** — 이쪽은 부채 회수 비율이다.
    """
...
def weighted_funding_rate(debts: list[tuple[float, float]]) -> float:
    ...
    ⚠⚠ 이 규칙을 안 쓰면 답이 틀린다 — tests/가 그 함정을 지킨다.
...
def rate_sensitivity(
    ...
    자산수익률 칸이 **한 번도 안 움직이는 것**이 이 표의 요점이다 —
    대출은 물건의 수익을 키우지 않는다.
    """
    if loan is None:
        loan = asset - equity
