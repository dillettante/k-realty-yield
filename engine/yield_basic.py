...
『돈공부』 3.A.3의 계산을 그대로 옮긴다. 새로 설계한 식은 없다.
- §판정 1  한 낱말이 다섯 가지 셈법을 가리킨다
- §판정 2  표면 5%짜리에서 실제로 남는 것은 2.76%다 (단계 분해)
- §판정 3  공실을 0으로 두지 않는다
...
MAN = 10_000  # 만원. 책의 표가 만원 단위라 대조할 때 쓴다.
...
@dataclass
class Step:
    """단계 분해의 한 줄. 책 §판정 2의 표 한 행에 대응한다."""
...
def surface_yield(p: Property) -> float:
    ...
    ⚠ 서재 45권에 「표면수익률」이라는 낱말 자체가 0회다(§판정 1).
    """
...
def net_yield_steps(p: Property) -> list[Step]:
    """표면 → 실질로 가는 단계 분해. 책 §판정 2의 표를 그대로 만든다.

    빼는 순서가 각 칸의 크기를 바꾼다(합계는 순서와 무관).
    책이 쓴 순서를 그대로 따른다 — 부대비용 → 공실 → 보유운영비 → 재계약비 → 세금.
    """
    steps: list[Step] = []
...
def net_yield(p: Property) -> float:
    ...
    ⚠ 물가는 빼지 않았다. 이 값은 명목이다(책 3.A.3 하단 메모).
    """
...
def roi_leveraged(p: Property) -> float:
    ...
    ⚠ 서재에서 **이자를 뺀 유일한 부동산 수익률 계산**이다(§판정 1).
    """
...
def five_measures(p: Property) -> dict[str, float | None]:
    ...
    시세차익 계열 둘(송희창·너나위)은 매도 시세가 있어야 성립하므로
    여기서는 None으로 두고 acquisition.py가 받는다.
    """
    return {
        "표면수익률(안민석)": surface_yield(p),
        "실질수익률(이 책)": net_yield(p),
